...
for (const antenna of antennas.keys()) {
  ...
  for (let i = 0; i < antennaLocations.length; i++) {
    for (let j = 0; j < antennaLocations.length; j++) {
      ...
      interferences[location1.y][location1.x] = true
      interferences[location2.y][location2.x] = true
      ...
      let iterator = 1
      while (true) {
        const newY = location1.y + dy * iterator
        const newX = location1.x + dx * iterator
